class UploadTools {
  static getFile = (event, callback) => {
    ...
    if ("files" in input && input.files.length > 0) {
      UploadTools.sendFileContent(callback, input.files[0], input.value);
    }
  };

  static sendFileContent = (callback, file, filename) => {
    UploadTools.readFileContent(file)
      .then((content) => {
...
class UploadButton {
  static create = (
    ...
    let inputId = `${elId}_input`;
    container.append(
      `<input type='file' id='${inputId}' style="display: none; visibility: hidden;"></input>`
    );
    container.append(
    ...
    input.change(e => {UploadTools.getFile(e, callback); } );
  };
}
...
class Dropzone {
  ...
  static getFile = (e, callback) => {
    UploadTools.getFile(e, callback);
  }
  ...
  static getDroppedFile = (e, callback) => {
    ...
    if (oe.dataTransfer.files.length) {
      UploadTools.sendFileContent(callback, oe.dataTransfer.files[0], oe.dataTransfer.files[0].name);
    }
  };
}
